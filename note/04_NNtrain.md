...
1. **정확도**라는 지표를 쓴다면? 100개의 test set 중에서 32개가 정답이여서 32%의 정확도라는 지표를 가지고 있다. 우리가 다음에 모델을 개선시키면 정확도는 32.011% 가 아닌, 34%, 35%로 _**불연속적인 미분의 값**_ 으로 학습할 것이다. 즉, 34~35 사이에 값은 우리가 알수 없는 것이다.
